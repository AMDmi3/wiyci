...
pub struct PollingWorkerRunner<Task, GetTask, ProcessTask> {
    ...
    span_from_task: Option<fn(&Task) -> Span>,
    task_wait: Duration,
    retry_wait: Duration,
}

impl<Task, Error, GetTask, ProcessTask> PollingWorkerRunner<Task, GetTask, ProcessTask>
where
    Error: std::fmt::Display,
    GetTask: AsyncFn() -> Result<Option<Task>, Error>,
    ProcessTask: AsyncFn(&Task) -> Result<(), Error>,
{
    pub fn new(name: &'static str, get_task: GetTask, process_task: ProcessTask) -> Self {
        Self {
            name,
            get_task,
            process_task,
    ...
    #[allow(unused)]
    pub fn with_span(mut self, span_from_task: fn(&Task) -> Span) -> Self {
        self.span_from_task = Some(span_from_task);
        self
    }
        ...
        ProcessTask: AsyncFn(&Task) -> Result<(), Error>,
    {
        loop {
            ...
            let res = async {
                ...
                let res = (self.process_task)(&task).await;
